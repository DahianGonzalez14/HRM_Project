...
namespace ProyectoFinalASP.Controllers
{
    public class SalidasController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "ID,FK_Empleado,Tipo_Salida,Motivo,FechaSalida")] Salida salida)
        {
            int m = Convert.ToInt32(salida.FK_Empleado);
            var id = (from s in db.Empleados where s.ID == m select s).First();
            id.Estatus = "Inactivo";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,FK_Empleado,Tipo_Salida,Motivo,FechaSalida")] Salida salida)
        {
            int m = Convert.ToInt32(salida.FK_Empleado);
            var id = (from s in db.Empleados where s.ID == m select s).First();
            id.Estatus = "Inactivo";
